...
let kintoneRestApiClient: KintoneRestAPIClient | undefined;
...
export const ktClient = async () => kt().then(kintoneClient => kintoneClient);

export const ktRecord = async () => ktClient().then(({record}) => record);

export const ktApp = async () => ktClient().then(({app}) => app);
